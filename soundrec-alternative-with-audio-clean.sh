...
case ${operation} in
	...
	*)
		...
		processaudio2="processaudio2_$now"
		...
		# Clean audio due to shitty microphone
		ffmpeg -y -i $tempaudiofolder/$processaudio -af "afftdn=nf=-25" $tempaudiofolder/$processaudio2
		ffmpeg -y -i $tempaudiofolder/$processaudio2 -af "afftdn=nf=-25" $tempaudiofolder/$processaudio
		ffmpeg -y -i $tempaudiofolder/$processaudio -af "afftdn=nf=-25" $tempaudiofolder/$processaudio2
		ffmpeg -y -i $tempaudiofolder/$processaudio2 -af "afftdn=nf=-25" $tempaudiofolder/$processaudio
		ffmpeg -y -i $tempaudiofolder/$processaudio -af "highpass=f=200, lowpass=f=3000" $tempaudiofolder/$processaudio2
		ffmpeg -y -i $tempaudiofolder/$processaudio2 -af "volume=12" $tempaudiofolder/$processaudio

		# To sync audio with video - In my case: add 2 seconds of silence before the audio stream
		ffmpeg -y -i $tempaudiofolder/$processaudio -af "adelay=$audiodelay|$audiodelay" $tempaudiofolder/$readyaudio		
