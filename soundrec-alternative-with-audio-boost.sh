...
audiodelay=1000								# milliseconds: Audio delay: used to sync audio and video
...
case ${operation} in
	...
	*)
		...
		ffmpeg -y -i $tempaudiofolder/$processaudio -af "volume=2.5" $tempaudiofolder/$processaudio2

		# To sync audio with video - In my case: add 2 seconds of silence before the audio stream
		ffmpeg -y -i $tempaudiofolder/$processaudio2 -af "adelay=$audiodelay|$audiodelay" $tempaudiofolder/$readyaudio		
		rm $tempaudiofolder/$processaudio || true	
		rm $tempaudiofolder/$processaudio2 || true	
